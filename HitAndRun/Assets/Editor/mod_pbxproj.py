...
class XCBuildConfiguration(PBXType):
    ...
    def add_other_buildsetting(self, flag, value):
        ...
        if not self.has_key(base):
            ...
            #print "Has key " + key + " and value: " + self[base][key] + " --> " + value
        self[base][key] = value
        modified = True
        return modified
...
class XcodeProject(PBXDict):
    ...
    def add_run_script(self, target, script=None, atBeginning=False):
        result = []
    ...
    def add_run_script_all_targets(self, script=None, atBeginning=False):
        result = []
    ...
    def add_file(self, f_path, parent=None, tree='SOURCE_ROOT', create_build_files=True, weak=False, ignore_unknown_type=False):
        ...
        if file_ref.build_phase and create_build_files:
            ...
            if tree == 'SOURCE_ROOT' and file_ref.build_phase == 'PBXFrameworksBuildPhase':
                if abs_path:
                    ...
                    if os.path.isfile(abs_path):
                        ...
                        print('Doing nothing with file ' + f_path)
                    elif f_path.endswith('.framework'):
                        #framework_path = os.path.join('$(SRCROOT)', f_path))
                        #framework_path = os.path.join('$(SRCROOT)', os.path.split(f_path)[0])
                        self.add_framework_search_paths(['$(PROJECT_DIR)'], recursive=False)
                    else:
                        # no documented behavior
                        print('Doing nothing with dir ' + f_path)
